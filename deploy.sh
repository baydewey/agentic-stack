...
#   ./deploy.sh [DASHBOARD_USER] [DASHBOARD_PASS] [ROUTER_PASS]
...
# .env.9router (REQUIRED - generates secrets if missing)
if [[ ! -f .env.9router ]]; then
...
# ── Auto-create Hermes dashboard config (basic auth) ────────────────────────
mkdir -p config
...
# Dashboard Auth - Basic Auth (scrypt format)
dashboard:
  basic_auth:
    username: "${DASH_USER}"
    password_hash: "${DASH_HASH}"
EOF
  echo "✅ Created config/dashboard.yaml"
else
  echo "⚠️  config/dashboard.yaml already exists — SKIP (hapus dulu untuk regenerate)"
fi
